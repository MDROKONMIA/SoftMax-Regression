...
$$\ P\left( y = k \middle| x \right) = \frac{e^{xw_{i}}}{\sum_{j = 1}^{k}e^{{xw}_{j}}}$$

Where $$\ w_{k} =\begin{bmatrix}
\begin{matrix}
\ldots & w_{0k} & \ldots \\
\end{matrix} \\
\begin{matrix}
...
$$\mathrm{1} \lbrace .\rbrace = \begin{cases}
    1 & \text{if } y = i \\
    0 & \text{otherwise.}
\end{cases}$$
...
of K classes, $$y^{(i)} \in \ \lbrace 1,\ 2,\ \ldots,\ K\rbrace.$$

To get the likelihood on the training data, we need to compute all of
the probabilities of $$y = y^{(i)}$$ given $$x^{(i)}$$ for $$i=1, 2, 3, ..., N.$$ ($$N$$
...
matrix, where each column $$\beta_{(k)}$$ corresponds to parameters for
...
Here $$\sum_{k = 1}^{K}{1\left\{ y^{(i)} = k \right\}} = 1$$, because only one
